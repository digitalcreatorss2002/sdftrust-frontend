...
const BeforeAfterImpact = () => {
  ...
  useEffect(() => {
    const fetchImpactData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/get_dual_images.php?t=${Date.now()}`);
  ...
  return (
    <section className="py-16 bg-[#F8F7F3] relative before-after-slider">
      {/* स्वाइपर बटन्स और बुलेट्स के लिए SDF थीम स्टाइल */}
      <style>{`
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-12 md:px-16">
        ...
        >
          {impactData.map((item) => {
            const img1Paragraphs = [item.img1_p1, item.img1_p2, item.img1_p3, item.img1_p4].filter(Boolean);
            const img2Paragraphs = [item.img2_p1, item.img2_p2, item.img2_p3, item.img2_p4].filter(Boolean);
            ...
            return (
              <SwiperSlide key={item.id} className="p-1">
                <div className="w-full bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-100 flex flex-col relative">
                  ...
                  {/* TOP HEADER */}
                  <div className="bg-[#0D3B66] text-white py-5 px-6 text-center shadow-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 relative">
                    ...
                    <div className="flex flex-col border-b md:border-b-0 md:border-r border-gray-200">
                      ...
                      <div className="relative aspect-[4/3] w-full overflow-hidden">
                        <span className="absolute top-1 left-1/2 -translate-x-1/2 bg-[#C92A2A] text-white text-xs sm:text-sm font-bold px-6 py-1.5 rounded-full shadow-md z-10 tracking-wider">
                          BEFORE
                        </span>
                        <img 
                      ...
                      <div className="p-6 md:p-8 bg-[#FAFAFA] flex-grow pt-10">
                        <ul className="space-y-3">
                          {img1Paragraphs.map((p, idx) => (
                            <li key={`p1-${idx}`} className="flex items-start gap-2.5 text-gray-700 font-medium text-sm md:text-base">
                              <span className="text-[#C92A2A] text-lg leading-none select-none">•</span>
                              <span>{p}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    {/* दक्षिण पक्ष (RIGHT: AFTER SECTION) */}
                    <div className="flex flex-col">
                      {/* इमेज कंटेनर */}
                      <div className="relative aspect-[4/3] w-full overflow-hidden">
                        <span className="absolute top-1 left-1/2 -translate-x-1/2 bg-[#2B8A3E] text-white text-xs sm:text-sm font-bold px-6 py-1.5 rounded-full shadow-md z-10 tracking-wider">
                          AFTER
                        </span>
                        <img 
                      ...
                      <div className="p-6 md:p-8 bg-[#F4F9F4] flex-grow pt-10">
                        <ul className="space-y-3">
                          {img2Paragraphs.map((p, idx) => (
                            <li key={`p2-${idx}`} className="flex items-start gap-2.5 text-gray-800 font-medium text-sm md:text-base">
                              <span className="text-[#2B8A3E] text-lg leading-none select-none">•</span>
                              <span>{p}</span>
                            </li>
                          ))}
                        </ul>
