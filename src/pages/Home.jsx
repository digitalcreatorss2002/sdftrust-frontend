  ...
  useEffect(() => {
    ...
    const fetchHomeAboutImages = async () => {
      try {
        ...
        if (resData.status === "success" && Array.isArray(resData.data)) {
          setGalleryImages(resData.data.slice(0, 4));
        }
      } catch (err) {
        console.error("Failed to fetch gallery row images:", err);
      } finally {
    ...
    const fetchPartnersDataDirectly = async () => {
      try {
        ...
        let combined = [];
        if (res1.status === "success" && Array.isArray(res1.data))
          combined = [...combined, ...res1.data];
        if (res2.status === "success" && Array.isArray(res2.data))
          combined = [...combined, ...res2.data];
        if (res3.status === "success" && Array.isArray(res3.data))
          combined = [...combined, ...res3.data];
    ...
    <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
                {galleryLoading ? (
                  <div className="col-span-1 sm:col-span-2 xl:col-span-4 text-center text-gray-500 py-10 bg-white/50 rounded-2xl border border-dashed">
                    <p className="font-medium">Loading images...</p>
                  </div>
                ) : galleryImages.length > 0 ? (
                  galleryImages.map((imgItem, idx) => (
                    <div
                      key={imgItem.id || idx}
                      className="bg-white rounded-2xl shadow-sm text-left border border-gray-100 pb-4 flex flex-col h-full hover:shadow-md transition-shadow overflow-hidden"
                    >
                      <div className="p-4 h-66 w-full">
                        <img
                          src={getGalleryImageUrl(imgItem.image_path)}
                          alt="SDF Asset"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
                  {focusAreas && focusAreas.length > 0 ? (
                    focusAreas.map((area, index) => (
                      ...
                      >
                        <div
                          className={`w-11 h-11 rounded-full flex items-center justify-center text-2xl bg-gray-50 shadow-sm shrink-0 ${area.color_class} ${area.animation_class}`}
                        >
                          {area.icon}
                        </div>
                  ...
                  <div className="mb-8 rounded-xl overflow-hidden h-54 bg-gray-100 border border-gray-100 shadow-inner">
                    <img
                      src={
                        ...
                        "https://placehold.co/400x250?text=SDF+Impact"
                      }
      ...
      <section className="py-10 bg-primary/10 border-t border-primary/20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          {message.text && (
            <div
              className={`max-w-lg mx-auto mt-4 p-3 rounded-lg text-sm font-medium ${message.type === "success" ? "bg-green-100 text-green-800 border border-green-200" : "bg-red-100 text-red-800 border border-red-200"}`}
            >
              {message.text}
            </div>
          )}
