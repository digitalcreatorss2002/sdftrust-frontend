...
const Projects = () => {
  ...
  const globalCompletedCount = projects.filter(
  ...
  if (activeTab === "all") {
    displayProjects = projects.filter(
      (p) =>
        ...
        p.status?.trim().toLowerCase() === "ongoing",
    );
    if (selectedCategory) {
    ...
    displayProjects = projects.filter(
      (p) => p.status?.trim().toLowerCase() === "planned",
    );
    if (selectedCategory) {
      displayProjects = displayProjects.filter(
        (p) => p.category?.trim() === selectedCategory,
  ...
  return (
    <div className="bg-bg-color min-h-screen pb-20">
      <section id="ongoing" className="bg-primary text-white py-35 relative overflow-hidden">
        <div
          className="absolute inset-0 z-0 bg-[url('/header/our-work.webp.jpeg')] bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: `url('header/our-work.webp.jpeg')` }}
        />
      ...
      <section className="border-b sticky top-20 bg-white z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative group">
          ...
          <div
            ref={scrollRef}
            className="flex items-center justify-center space-x-8 overflow-x-auto no-scrollbar scroll-smooth px-12"
          >
            <button
              onClick={() => {
                setActiveTab("all");
              ...
              className={`py-4 border-b-2 font-bold whitespace-nowrap transition-colors shrink-0 ${activeTab === "all" ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-primary"}`}
            >
              Ongoing Projects 🏢
            </button>
            <button
              onClick={() => {
                setActiveTab("completed");
                setSelectedCategory(null);
                window.history.replaceState(null, "", `#completed`);
              }}
              className={`py-4 border-b-2 font-bold whitespace-nowrap transition-colors shrink-0 ${activeTab === "completed" ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-primary"}`}
            >
              Completed Projects ✅
            </button>
            <button
              ...
              className={`py-4 border-b-2 font-bold whitespace-nowrap transition-colors shrink-0 ${activeTab === "planned" ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-primary"}`}
            >
              Planned Projects 📋
            </button>
            <button
              ...
              className={`py-4 border-b-2 font-bold whitespace-nowrap transition-colors shrink-0 ${activeTab === "listings" ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-primary"}`}
            >
              State-wise Listings 📍
            </button>
          </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mt-2 pb-4 bg-white border-t pt-3 border-gray-50 animate-in fade-in duration-300">
                {uniqueCategories.map((cat) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedCategory === cat ? "bg-[#6a752b] text-white shadow" : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200"}`}
                  >
                    {formatTabLabel(cat)}
                  </button>
                ))}
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mt-2 pb-4 bg-white border-t pt-3 border-gray-50 animate-in fade-in duration-300">
              {uniqueStates.map((state) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeState === state ? "bg-[#6a752b] text-white shadow" : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200"}`}
                >
                  {state}
                </button>
              ))}
                ...
                >
                    ...
                    {isVideo ? (
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          e.target.src =
                            "https://placehold.co/600x400?text=Media+Not+Found";
                        }}
                      />
                    )}
                  </div>
                  <div className="flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`text-[9px] uppercase font-bold px-2 py-0.5 rounded ${project.status?.trim().toLowerCase() === "completed" ? "bg-green-100 text-green-700" : project.status?.trim().toLowerCase() === "planned" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-700"}`}
                      >
                        {project.status?.trim().toLowerCase() === "active" ||
                        project.status?.trim().toLowerCase() === "ongoing"
                          ? "Ongoing"
                          : project.status}
                      </span>
                    </div>
                    <h3 className="text-2xl font-serif font-bold text-text-primary mb-2 leading-tight">
                      {project.title}
                    </h3>
                    ...
                    <Link
                      ...
                      className="inline-block bg-primary hover:bg-[#5a6425] text-white px-5 py-2 rounded font-medium text-sm transition-colors text-center self-start shadow-sm"
                    >
                      View Details
                    </Link>
                  </div>
                  ...
                  <div className="mb-8 rounded-xl overflow-hidden h-54 bg-gray-100 border border-gray-100 shadow-inner">
                    <img
                      src={
                        ...
                        "https://placehold.co/600x400?text=SDF+Impact"
                      }
