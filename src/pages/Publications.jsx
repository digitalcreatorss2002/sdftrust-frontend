...
const Publications = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex justify-center items-center text-primary font-bold animate-pulse">
  ...
  if (error) {
    return (
      <div className="text-center text-red-500 py-20 font-bold">{error}</div>
    );
  }

  const reports = publications.filter((p) => p.type === "report");
  const caseStudies = publications.filter((p) => p.type === "case_study");
  const legalDocuments = publications.filter(
    (p) => p.type === "legal_document",
  );
  ...
  return (
    <div className="bg-bg-color min-h-screen">
      <section className="bg-primary text-white py-35 px-4 text-center relative overflow-hidden">
        <div
          className="absolute inset-0 z-0 bg-[url('header/publication.webp.jpeg')] bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: `url('header/publication.webp.jpeg')` }} // Aap apna sahi image path yahan replace kar sakte hain
        />
        ...
        <div className="max-w-7xl mx-auto relative z-20">
          <motion.h1
            ...
            className="text-4xl md:text-5xl font-bold mb-4 font-serif drop-shadow-md"
          >
          ...
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-white opacity-90 max-w-2xl mx-auto text-lg drop-shadow-sm"
          >
            Explore our reports, case studies, legal documents, and books
            showcasing our impact-driven work.
          </motion.p>
        </div>
      </section>
              ...
              {reports.map((r) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-red-50 text-red-500 rounded flex items-center justify-center font-bold shrink-0">
                      PDF
                    </div>
                    <div>
          ...
          (caseStudies.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {caseStudies.map((c) => (
                <div
                  key={c.id}
                  className="bg-white rounded-xl overflow-hidden shadow-sm hover:-translate-y-1 transition-transform border border-gray-100 flex flex-col"
                >
                  <div className="h-48 bg-gray-200">
                    <img
                      src={
                        getFullUrl(c.image_url) ||
                        "https://images.unsplash.com/photo-1544027993-37dbddc92582?q=80&w=400&auto=format&fit=crop"
                      }
                      alt={c.title}
                      className="w-full h-full object-cover"
                      onError={(e) => {
                        e.currentTarget.src =
                          "https://via.placeholder.com/400x250?text=SDF+Case+Study";
                      }}
                    />
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="font-bold text-gray-900 text-lg flex-1 mb-4 leading-tight">
                      {c.title}
                    </h3>
                    {c.file_url && (
                      <a
                        href={getFullUrl(c.file_url)}
                        target="_blank"
                        rel="noreferrer"
                        className="text-primary hover:text-[#5a6425] text-sm font-bold flex items-center gap-1 mt-auto"
                      >
                        Read Study <span className="text-lg">→</span>
                      </a>
                    )}
                  </div>
                </div>
          ...
          (inPublications.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {inPublications.map((book) => (
                <div
                  key={book.id}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:-translate-y-1 hover:shadow-md transition-all flex flex-col"
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between items-center text-center">
                    <h3 className="font-bold text-base text-gray-900 mb-4 leading-tight line-clamp-2">
                      {book.title}
                    </h3>
                    {book.pdf_url && (
                      <a
                        href={getFullUrl(book.pdf_url)}
                        target="_blank"
                        rel="noreferrer"
                        className="bg-[#6a752b] text-white hover:bg-[#5a6425] w-full py-2.5 rounded text-sm font-bold shadow-sm transition-colors block mt-auto"
                      >
                        Read PDF
                      </a>
                    )}
